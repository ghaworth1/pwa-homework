...
export const putDb = async (content) => {
  ...
  const allCaps = await openDB(DB_NAME, 1);

  // TODO: Now create a variable for the transaction; again, this will be referenced below.
  const changeText = allCaps.transaction(DB_NAME, 'readwrite');

  // TODO: Now create a variable for the store
  const myStore = changeText.objectStore(DB_NAME);
...
export const getDb = async () => {
  ...
  const allCaps = await openDB(DB_NAME, 1);
  const changeText = allCaps.transaction(DB_NAME, 'readonly');
  const myStore = changeText.objectStore(DB_NAME);
